import { parseJsonBody, saveDashboardSettings, verifyDashboardRequest } from '../_lib/daybird.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await saveDashboardSettings(auth.ownerUid, parseJsonBody(req).weights);
    return res.status(200).json({ ok: true, ...result });
  } catch (error) {
    console.error('[daybird:settings]', error);
