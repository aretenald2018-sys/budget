...
import {
  ...
  mergeReceiptMemo,
  normalizeReceiptText as normalizeText,
...
export async function processReceipt(parsed, emailId) {
  ...
  if (match) {
    ...
    if (parsed.merchant && isGenericReceiptMerchant(match.data().merchant || match.data().counterparty)) {
...
async function linkReceiptToTransaction(receiptRef, txRef, receiptId, receipt, parsed) {
  ...
  if (parsed.merchant && isGenericReceiptMerchant(txData.merchant || txData.counterparty)) {
    patch.merchant = parsed.merchant;
  }
