...
async function searchGoogleCustomImages(query, limit = 8) {
  ...
  return (data.items || []).slice(0, limit).map((item, index) => ({
    label: plainText(item.title || `${query} 이미지 ${index + 1}`).slice(0, 90),
...
async function searchPexels(query) {
  const url = `https://api.pexels.com/v1/search?query=${encodeURIComponent(toEnglishQuery(query))}&per_page=8&orientation=portrait`;
...
async function searchPixabay(query) {
  const url = `https://pixabay.com/api/?key=${encodeURIComponent(process.env.PIXABAY_API_KEY)}&q=${encodeURIComponent(toEnglishQuery(query))}&image_type=photo&orientation=vertical&safesearch=true&per_page=8`;
  const response = await fetch(url);
  if (!response.ok) throw new Error(`pixabay ${response.status}`);
  const data = await response.json();
  return (data.hits || []).map(photo => ({
    label: photo.tags || query,
