...
flowchart TD
    ...
    Home -->|"나의 목표 전체 보기 / 설정하기"| Goals
    Home -->|"KPI 고정비·예산·충당금"| Goals
    Home -->|"KPI 수입"| Tx
    Settings -->|"메뉴 행"| Finance
    Settings -->|"메뉴 행"| Settle
```

| 화면 | 구현 상태 | 연결된 계약서 |
| --- | --- | --- |
| 홈(허브, 스크린샷 구조로 개편) | 프로토타입 | `docs/ai/contracts/home.contract.md` (개편 반영 갱신 필요) |
| 목표 탭 | 프로토타입 (신설) | _(경화 시 작성)_ |
| 검색 오버레이 | 스텁 | _(경화 시 작성)_ |
| 거래·설정·검토·리포트·자산·정산 | 기존 | — |
...
| KPI 충당금 | 목표 탭 충당금 섹션 | 설정 탭 충당금 섹션 | `가정` (목표 탭 신설에 따름) |
| KPI 고정비 | 목표 탭 | 리포트 탭 ("이상한" 연결 — 고정비 맥락 없음) | `가정` |
| KPI 이번 달 예산 | 목표 탭 | 설정 탭 | `가정` |
| 지출 요약 "전체 보기" | 리포트 탭 | 동일 | 유지. 카드 타이틀 "지출 카테고리"→"지출 요약" |
| 지출 요약 범례 행 | 카테고리 거래 모달 | 동일 (`open-category`, 기타 비클릭) | 유지 |
| 나의 목표 "전체 보기" | 목표 탭 | 설정 탭 | 변경 |
| 목표 카드 | 목표 상세 모달 | 동일 (`open-goal-detail`) | 유지 |
| 미분류 "설정하기" | 목표 탭 | 설정 탭 | 변경 |
| 포인트 행 / "자세히 보기" | 포인트 모달 | 동일 | 유지 |
| 하단 내비 | 홈 · 목표 · (+) · 거래 · 설정 | 홈 · 자산 · (+) · 거래 · 검토 (설정은 헤더 톱니) | 개편 |
| + FAB | 거래 추가 모달 | 동일 (`openTxAddModal`) | 유지 |

## 3. 목표 탭 (신설 프로토타입) 구성

기존 데이터 모델 재사용으로 조립한다 — 새 계산 로직을 만들지 않는다.

- 카테고리 목표 그리드: 홈의 `buildGoals` 결과 재사용(전체 노출 + 목표 상세 모달 연결).
- 이번 달 예산 요약: 설정 탭의 예산 요약 데이터 재사용.
- 충당금 목록: 홈 `buildFundsSection` 재사용(+ 만들기 CTA → 기존 충당금 설정 진입).
- 미확정 세부(목표 편집 UI 등)는 스텁("준비중").

## 4. 미확정 연결 (객관식 — 전항 `가정` 적용 중)

### Q1. 목표 탭의 정체
(a) 목표 허브 신설(카테고리 목표+예산+충당금) **← 추천·가정 적용** / (b) 기존 리포트 탭 개편 / (c) 스텁만

### Q2. 내비에서 빠지는 탭들의 진입점
(a) 자산·정산=설정 메뉴 행, 검토=종 아이콘, 리포트=지출 요약 전체 보기 **← 추천·가정 적용** / (b) 홈 카드로만 / (c) 일부 탭 삭제
