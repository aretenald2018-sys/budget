...
- 실행 슬라이스: 실행 슬라이스 1 - 홈 모드 버튼 렌더 대상 및 월 MAX 게이지 폭 고정
...
- `app.js`, `index.html`
  - 새 JS가 운영에서 로드되도록 cache-busting query를 `20260702-home-month-gauge-fix`로 갱신했다.
- `styles/60-urge.css`, `style.css`, `index.html`
  ...
  - 새 CSS가 운영에서 로드되도록 `style.css`와 `styles/60-urge.css` cache-busting query를 `20260702-home-month-gauge-fix`로 갱신했다.
...
- `_site` 확인
  - `style.css?v=20260702-home-month-gauge-fix` 반영 확인.
  - `styles/60-urge.css?v=20260702-home-month-gauge-fix` 반영 확인.
  - `app.js?v=20260702-home-month-gauge-fix` 반영 확인.
  - `render-report.js?v=20260702-home-month-gauge-fix` 반영 확인.
