...
- 완료: GitHub Pages 배포 후 운영 URL에서 홈 카드 스타일과 설정 저장 흐름을 직접 확인했다.
- 커밋: `a21625d Expand reward home system settings`
- Pages workflow: `Deploy GitHub Pages` run `28575288662` 성공.
- 운영 홈 확인:
  - `오늘의 적립`, `이번 2주 변동비`, `이번 달 고정비`, `Dev Ideas` 모두 배경 `rgb(32, 38, 45)`, 텍스트 `rgb(255, 255, 255)`, radius `20px`.
  - 홈 변동비 게이지 fill `rgb(243, 183, 200)`.
- 운영 설정 확인:
  - `보상 적립` 폼 표시.
  - 기본값: 기준 기간 `180`, 기준선 방식 `trimmed_weekly`, 배분율 `30`, 일 상한 `10000`, 월 상한 `120000`.
  - 배분율을 `10%`로 저장하면 홈 카드가 `10% 배분`으로 변경됨.
  - 다시 `30%`로 저장해 원복했고 홈 카드가 `30% 배분`으로 표시됨.
