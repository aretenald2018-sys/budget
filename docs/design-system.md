...
### 5-11. 홈 탭 — 다크 대시보드 레이어 ★

홈 탭은 2026-07 재구축(프리미엄 다크 대시보드) 이후 **앱에서 유일하게 전체 다크 캔버스**를 쓴다. "라이트 베이스 + 다크 히어로 한 장" 원칙(§7-1)의 명시적 예외이며, 홈 밖 탭에는 이 레이어를 확장하지 않는다.

**스코프 규칙:**
- 홈 다크 토큰(`--hd-*`)과 스타일은 전부 `styles/features/home-dashboard.css` 안에서 `.home-dash` / `#tab-home` / `body[data-tab="home"]`로 스코프한다.
- 전역 `:root` 팔레트에 홈 다크 값을 섞지 않는다. 반대로 홈 안에서 전역 라이트 토큰을 배경/표면에 쓰지 않는다.
- 캔버스 `#0A0C12`, 표면 `#14161F`/`#1E2230`, 브랜드 `--hd-brand #7C5CF0`.
- 홈에 진입하면 전역 앱 헤더는 숨겨지고 대시보드 자체 헤더(`hd-header`)가 그 역할을 대신한다.
- 하단 내비는 홈에서 색만 다크 톤으로 재조정한다. **구성(슬롯 수·버튼 순서)은 다른 탭과 동일해야 한다.**
- 리포트 탭(홈 밖)은 계속 라이트 규칙(§1~§7)을 따른다. 홈과 리포트가 다른 세계관인 것은 의도된 상태다.
